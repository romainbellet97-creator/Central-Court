...
async def check_tournament_conflicts(tournament_id: str, user: dict = Depends(require_auth)):
    """Check if a tournament conflicts with the authenticated user's calendar events"""
    ...
    # Find overlapping events scoped to this player
    ...
    # Check for conflicting registered tournaments scoped to this player
    conflicting_tournaments = []
    regs = await db.tournament_registrations.find(
        {"player_id": player_id, "status": {"$in": ["pending", "accepted", "participating", "interested"]}},
        {"_id": 0}
    ).to_list(200)
...
async def list_tournaments(
    ...
    """List all tournaments with filters (public — tournament catalog)."""
    query = {}
...
async def list_user_tournaments(
    user_id: str,
    limit: int = Query(default=100, le=500),
    user: dict = Depends(require_auth)
):
    """Get tournaments based on the authenticated user's circuit preferences"""
    ...
    user_circuits = user_doc.get("circuits", [])
...
async def list_tournament_weeks(
    ...
    """Get tournaments grouped by week with the authenticated user's registrations and hidden status."""
    player_id = user["user_id"]
    ...
    # Scope registrations and hidden to the authenticated player
...
async def get_tournament_stats():
    """Get aggregate statistics about available tournaments (public)"""
    pipeline = [
        {"$group": {
            "_id": "$circuit",
            "count": {"$sum": 1}
...
async def register_tournament(req: RegisterTournamentRequest, user: dict = Depends(require_auth)):
    """Register or update registration for a tournament (scoped to authenticated player)"""
    player_id = user["user_id"]
    ...
    await db.tournament_registrations.update_one(
        {"player_id": player_id, "tournamentId": req.tournamentId},
        {"$set": {
            "player_id": player_id,
    ...
    # If participating, hide other tournaments in same week for this player
    if req.status == "participating":
        ...
        for t in week_tournaments:
            await db.tournament_hidden.update_one(
                {"player_id": player_id, "tournamentId": t["id"]},
                {"$set": {"player_id": player_id, "tournamentId": t["id"]}},
                upsert=True
            )

    # Remove from hidden if registering
    await db.tournament_hidden.delete_one({"player_id": player_id, "tournamentId": req.tournamentId})

    return {"success": True, "tournamentId": req.tournamentId, "status": req.status}


@router.post("/hide")
async def hide_tournament(req: HideTournamentRequest, user: dict = Depends(require_auth)):
    """Hide a tournament for the authenticated player"""
    player_id = user["user_id"]
    await db.tournament_hidden.update_one(
        {"player_id": player_id, "tournamentId": req.tournamentId},
        {"$set": {"player_id": player_id, "tournamentId": req.tournamentId}},
        upsert=True
    )
    await db.tournament_registrations.delete_one({"player_id": player_id, "tournamentId": req.tournamentId})
    return {"success": True}


@router.delete("/hide/{tournament_id}")
async def unhide_tournament(tournament_id: str, user: dict = Depends(require_auth)):
    """Unhide a tournament for the authenticated player"""
    player_id = user["user_id"]
    await db.tournament_hidden.delete_one({"player_id": player_id, "tournamentId": tournament_id})
    return {"success": True}
